...
async function getFace(video, options) {
  faceResults = await faceapi.mtcnn(video, options);

  return faceResults;
}

function getMTCNNParams() {
  return {minFaceSize: 200};
}
  ...
  return meeting.getMediaStreams(mediaSettings, audioVideoInputDevices)
    ...
    .then(([localStream]) => {
      if (localStream && mediaSettings.sendVideo) {
        ...
        meetingStreamsLocalVideo.addEventListener('playing', () => {
          const context = meetingStreamsLocalCanvas.getContext('2d');
          const image = new Image();

          image.src = 'img/sunglasses.png';

          function step() {
            getFace(meetingStreamsLocalVideo, 0, 0);

            context.drawImage(meetingStreamsLocalVideo, 0, 0);
        ...
        localStream = meetingStreamsLocalCanvas.captureStream(30);
      }

      return {localStream: meetingStreamsLocalCanvas.captureStream(30)};
    })
    .catch((error) => {
...
function addMedia() {
  ...
  meeting.on('media:ready', (media) => {
    ...
    switch (media.type) {
      case 'local':
        meetingStreamsLocalVideo.srcObject = media.stream;
        break;
      case 'remoteVideo':
