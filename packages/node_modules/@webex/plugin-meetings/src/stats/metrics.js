...
export default class MediaMetrics extends StatelessWebexPlugin {
  ...
  constructor(attrs, options, specs) {
     ...
     * @memberof MediaMetrics
     */
    this.mqaInterval = specs.mqaInterval;
    /**
     * @instance
     * @type {Boolean}
     * @readonly
     ...
     * @memberof MediaMetrics
     */
    this.callback = specs.callback;
    this.meetingId = specs.meetingId;
  }
  ...
  checkData(id, sendRecv) {
    ...
    if (mqa && mqa.get().length >= this.mqaInterval) {
      const interval = mqa.getSlice(this.mqaInterval);

      if (this.callback) {
        this.callback(this.meetingId, id, interval);
      }
