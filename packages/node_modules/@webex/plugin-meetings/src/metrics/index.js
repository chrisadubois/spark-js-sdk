...
class Metrics {
  ...
  postEvent(options) {
    ...
    if (meeting) {
      ...
      if (event === eventType.MEDIA_QUALIY_EVENT) {
        data.event = event;
 ...
 */
    ...
    const payload = {
      ...
      origin: {
        ...
        clientInfo: {
          ...
          os: 'linux', // TODO: parse from bowser, save the OS and use as a global variable
          osVersion: 'NULL' // TODO: parse from bowser, save the version and use as a global var
        }
      },
      originTime: {
        triggered: new Date().toISOString()
  ...
  // TODO: we can add clientName to userAgent for CA to parse and add a config value
  getUserAgentInfo() {
    let userAgentOption;
    let browserInfo;
    let osInfo;
    ...
    if (bowser.platform && bowser.osversion) {
      osInfo = util.format('os=%s', `${bowser.platform}/${bowser.osversion.split('.')[0]}`);
    }
    if (browserInfo) {
