  ...
  */
  ...
  getAnalyzerMetricsPrePayload(options) {
    if (options) {
      ...
      if (options.type === 'MEDIA_QUALITY_ANALYZER') {
  ...
  sendMediaQualityAnalyzerMetrics(options) {
    options.type = 'MEDIA_QUALITY_ANALYZER';
  ...
  /**
   * Reference to the mediaMetrics object
   ...
   */
  autoStats() {
    const automaticMetrics = new StatsMetrics(this.attrs, this.options, {interval: this.config.stats.mediaDiagnosticsInterval});

    const stats = this.getStats(automaticMetrics.initialize(), true);

    automaticMetrics.setStats(stats);

    this.mediaMetrics = automaticMetrics;
  }
  ...
  startMediaQualityMetrics() {
    ...
    //       const k = automaticMetrics.stats.getSender(key).getMQA();
    //     }
    //     else if (key.endsWith('receiver')) {
    //       const k = automaticMetrics.stats.getReceiver(key).getMQA();
    //     }
    //   });
    // }, 30000);
    const automaticMetrics = new StatsMetrics(this.attrs, this.options, {mqaInterval: this.config.metrics.mqaMetricsInterval, callback: this.processMQAData, meetingId: this.id});
  ...
  processMQAData(meetingId, id, values) {
    if (!this.mqaProcessor) {
      this.mqaProcessor = new MQAProcessor();
    }
    this.mqaProcessor.process(id, values);
    if (this.mqaProcessor.isReady()) {
      Metrics.postEvent({event: 'client.mediaquality.event', meetingId, data: {intervalData: this.mqaProcessor.getData()}});
    }
  }
    ...
    return MeetingUtil.validateOptions(options)
      ...
      .then(() => Media.attachMedia(this.mediaProperties, this.id)
        .then((peerConnection) => {
          ...
          if (this.config.stats.autoSendMediaDiagnostics) {
            this.autoStats();
          }
