...
export default class Meeting extends StatelessWebexPlugin {
  ...
  */
  ...
  startMediaQualityMetrics() {
    const automaticMetrics = new StatsMetrics(this.attrs, this.options, {meetingId: this.id});
  ...
  processMQAData() {
    ...
    MQA_STATS.DATA_PLACEMENTS.forEach((key) => {
      if (this.mediaQualityMetrics && this.mediaQualityMetrics.stats) {
        ...
        if (key.toLowerCase().endsWith('sender')) {
          sendRecvData = this.mediaQualityMetrics.stats.getSender(key);
        }
        else if (key.toLowerCase().endsWith('receiver')) {
        ...
        if (mqa) {
          const interval = mqa.getSlice(this.config.metrics.mqaMetricsInterval);
