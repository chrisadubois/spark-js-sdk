...
const State = StateMachine.factory({
  ...
  transitions: [
    {
      ...
      from: '*',
      to: function (signal, meeting, session, perform) {
        let value;
        switch (this.state) {
  ...
  methods: {
    onAfterStep: (transition, signal, session, meeting, perform) => {
      console.log(
  ...
  performState(session, meeting) {
    ...
    switch (session.state.state) {
      ...
      case session.state.state === ROAP.ROAP_STATE.GLARE:
        ...
        session.state.step(ROAP.ROAP_SIGNAL.GLARE_RESOLVED, meeting, session, this.performState);
        break;
      default:
        break;
    }
    ...
    switch (action.type) {
      case ROAP.RECEIVE_ROAP_MSG:
        ...
        if (this.save(action)) {
          ...
          session.state.step(signal, meeting, session, this.performState);
